...
def run(
    ...
    # Setup logging properly - console + file
    import logging
    ...
    # Console handler (INFO and above goes to stdout)
    ...
    file_handler = logging.FileHandler('rarb.log', mode='a')
    ...
    try:
        if realtime:
            from rarb.runner import run_realtime_bot
            asyncio.run(run_realtime_bot())
        else:
            from rarb.runner import run_bot
            asyncio.run(run_bot())
...
def scan() -> None:
    ...
    async def _scan() -> None:
        ...
        async with MarketScanner() as scanner:
            ...
            opportunities = await analyzer.analyze_batch(snapshots)
    ...
    async def _markets() -> None:
        ...
        async with GammaClient() as client:
            ...
            for market in markets[:limit]:
                yes_price = f"${float(market.yes_price):.2f}" if market.yes_price else "N/A"
                no_price = f"${float(market.no_price):.2f}" if market.no_price else "N/A"
                table.add_row(
                    market.question[:50],
...
@cli.command()
def tui() -> None:
    """Run the modern Textual TUI."""
    from rarb.tui.app import run_tui
    run_tui()
...
def dashboard(host: str, port: Optional[int]) -> None:
    ...
    from rarb.dashboard.app import run_dashboard
    run_dashboard(host=host, port=actual_port)


@cli.command()
@click.option("--strategy", type=click.Choice(["buy-above", "buy-opposite", "momentum"]), default="momentum")
@click.option("--asset", default="BTC", help="Asset to trade (BTC, ETH, SOL, XRP)")
@click.option("--threshold", type=float, default=0.95, help="Price threshold for Buy-Above/Buy-Opposite")
@click.option("--dry-run/--live", default=True, help="Dry run mode")
@click.option("--log-level", type=click.Choice(["DEBUG", "INFO", "WARNING", "ERROR"]), default="INFO")
def signal(
    strategy: str,
    ...
    """Run 5-minute market signal strategy."""
    import os
    ...
    mode = "[yellow]DRY RUN[/yellow]" if settings.dry_run else "[red]LIVE TRADING[/red]"
    console.print(f"\n[bold]5-Minute Signal Strategy[/bold] - {mode}")
    console.print(f"[bold]Strategy:[/bold] {strategy}")
    console.print(f"[bold]Asset:[/bold] {asset}")
    console.print(f"[bold]Threshold:[/bold] {threshold}\n")
    
    if not settings.dry_run:
        if not settings.private_key or not settings.wallet_address:
            console.print("[red]Error:[/red] Live trading requires credentials")
            sys.exit(1)
    
    async def _run():
        from rarb.strategies.five_minute import (
            BuyAboveStrategy,
            BuyOppositeStrategy,
            MomentumStrategy,
        )
        
        if strategy == "buy-above":
            strat = BuyAboveStrategy(threshold=threshold, asset=asset)
        elif strategy == "buy-opposite":
            strat = BuyOppositeStrategy(threshold=threshold, asset=asset)
        else:
            strat = MomentumStrategy(asset=asset)
        
        async def on_signal(result):
            console.print(f"[green]Signal:[/green] {result.direction} | {result.reason}")
        
        await strat.run(on_signal)
