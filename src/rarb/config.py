...
class Settings(BaseSettings):
    ...
    analyzer_type: str = Field(
        default="standard",
        description="AI provider: 'standard', 'gemini', 'openai', 'anthropic', 'groq', 'openrouter', 'nvidia', 'huggingface', 'xai', 'ollama', 'lmstudio'",
    )
    
    ai_confidence_threshold: float = Field(
        default=0.65,
        description="Minimum AI confidence (0-1) to approve trade",
        ge=0.0,
        le=1.0,
    )
    ...
    x_access_token_secret: Optional[SecretStr] = None
    ...
    gemini_model: str = Field(
        ...
        description="Gemini model for Vertex AI"
    )
    
    # Model Selections - OpenAI
    openai_model: str = Field(
        default="gpt-4o",
        description="OpenAI model: gpt-4o, gpt-4o-mini, o1-preview, o1-mini"
    )
    
    # Model Selections - Anthropic
    anthropic_model: str = Field(
        default="claude-3-5-sonnet-20241022",
        description="Anthropic model: claude-3-5-sonnet-20241022, claude-3-opus-20240229"
    )
    
    # Model Selections - Groq (Ultra-fast)
    groq_model: str = Field(
        default="llama-3.1-70b-versatile",
        description="Groq model: llama-3.1-70b-versatile, mixtral-8x7b-32768, llama-3.1-8b-instant"
    )
    
    # Model Selections - OpenRouter (50+ models)
    openrouter_model: str = Field(
        default="meta-llama/llama-3.1-405b-instruct",
        description="OpenRouter model path"
    )
    
    # Model Selections - NVIDIA
    nvidia_model: str = Field(
        default="nvidia/llama-3.1-nemotron-70b-instruct",
        description="NVIDIA model"
    )
    
    # Model Selections - xAI (Grok)
    xai_model: str = Field(
        default="grok-beta",
        description="xAI model: grok-beta, grok-vision-beta"
    )
    
    # Model Selections - HuggingFace
    huggingface_model: str = Field(
        default="meta-llama/Llama-3.1-70B-Instruct",
        description="HuggingFace model ID"
    )
    
    # Model Selections - Local (Ollama, LM Studio)
    local_model: str = Field(
        default="llama3.1:8b",
        description="Local model name for Ollama/LM Studio"
    )

    # Profitability & Custom Setups
    min_volume_24h: float = Field(default=1000.0, description="Min 24h volume to consider a market")
    smart_sizing: bool = Field(default=True, description="Use Kelly Criterion for sizing")
    use_sentiment_filter: bool = Field(default=True, description="AI must verify news sentiment")
    
    # New Alpha Features
    enable_auto_redeem: bool = Field(default=True, description="Auto-redeem won positions")
    ai_suggestion_frequency: int = Field(default=300, description="Seconds between AI deep-dives")
    max_daily_loss_usd: float = Field(default=50.0, description="Stop trading if daily loss exceeds this")
